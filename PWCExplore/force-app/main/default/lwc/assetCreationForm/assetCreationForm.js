...
export default class AssetCreationForm extends LightningElement {
    ...
    wiredPicklistValues({error,data}){
        if(data){
            this.areaCodeOptions = data.values.map(item=>{
    ...
    handleAreaCodeChange(event){
        ...
        console.log('AreaCode-->' , this.areaCodeValue);
    }


    handleProductChange(event){
        console.log('EventDetails-->',JSON.stringify(event.detail));
        this.productValue = event.detail.recordId;
    }
    ...
    createAsset() {
        createAsset({
            ...
            addressValue : this.areaCodeValue
        })
        .then(() => {
            this.resetForm();
